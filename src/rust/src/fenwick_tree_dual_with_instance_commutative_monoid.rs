...
where
    G::T: Clone,
{
    pub fn new(g: G, size: usize) -> Self { Self(Fenwick::new(g, size)) }

    pub fn size(&self) -> usize { self.0.size() }

    pub fn operate_ge(&mut self, i: usize, x: G::T) { self.0.operate(i, x) }

    pub fn get(&self, i: usize) -> G::T { self.0.fold_lt(i + 1) }
