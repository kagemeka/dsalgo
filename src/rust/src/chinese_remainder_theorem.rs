...
// }
pub fn crt(
    mut mod_0: u64, mut rem_0: u64, mut mod_1: u64, mut rem_1: u64,
    ...
    Ok((lcm, (x as u64 * s % lcm * mod_0 + rem_0) % lcm))
}
pub fn crt_coprime(
    mod_0: u64, rem_0: u64, mod_1: u64, rem_1: u64,
) -> (u64, u64) {
...
pub fn safe_crt(
    ...
    let (gcd, inv_u0) = mod_gcd_inv(mod_1 as u64, (mod_0 % mod_1) as u64);
    if (rem_1 - rem_0) % gcd != 0 {
        return Err("answer is undefined".to_string());
    }
    let u1 = mod_1 / gcd;
...
mod tests {
    ...
    fn test() {
        ...
        assert_eq!(crt_coprime(5, 3, 7, 4), (35, 18),);
        assert_eq!(safe_crt(5, 3, 7, 4), Ok((35, 18)),);
        let mod_rem_pairs = vec![(5, 3), (7, 4), (8, 3)];
        assert_eq!(safe_crt_reduce(&mod_rem_pairs), Ok((280, 123)),);
    }
}
